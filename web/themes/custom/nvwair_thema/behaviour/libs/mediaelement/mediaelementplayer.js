...
/* CUSTOM */
/* Om ondertiteling op IE9 werkend te krijgen is een custom xdr loader gemaakt */
	...
	mejs.MediaElementPlayer.prototype = {
		...
		setPoster: function(url) {
			...
			if ($posterImg.length === 0) {
				$posterImg = $('<img crossorigin="anonymous" width="100%" height="100%" src="'+url+'" alt="" />').load(function() {
					$posterImg.appendTo(posterDiv);
					// posterDiv.css({'background-image' : 'url(' + $posterImg.attr('src') + ')'});
					// alert('RO-Video no BG');
				});
			}
		},
				...
				error: function() {
					...
					if(window.XDomainRequest){
						...
						xdr.onload = function () { //process successful.
							...
							if (typeof response == "string" && (/<tt\s+xml/ig).exec(response)) {
								track.entries = mejs.TrackFormatParser.dfxp.parse(response);
							} else {
								track.entries = mejs.TrackFormatParser.webvtt.parse(response);
							}
